...
3.  **全面收集**: 涵蓋 System Info, Persistence (Autoruns/Tasks), Logs (Event/MFT/Registry), Browser History, File Artifacts (Lnk/Prefetch)。Registry 收集除 Hive／`ShellBags_<SID>.reg` 外，平台會自動產生第一階段結構化 **`Registry\shellbags.csv`**（供 Facts／Timeline／Entity search；非完整 ShellBags 鑑識還原器）。
...
| **V0_22_ENGINEERING_SPEC.md** | `v0.22.0 draft` 的工程級拆分文件：工作包、優先序、檔案邊界、驗收與 defer 項 |
...
> 若設定 Upload Endpoint，Local Collect 結束後會自動上傳 ZIP 並顯示回應。**v0.22.0**：Upload 與 AI 各有 endpoint **allowlist**（**Advanced → Settings**）；清單為空時預設阻擋對應出站 POST。AI 摘要外送可選 **redaction profile**（僅影響 AI Analyze 的 JSON，不會改寫匯出之 `summary.json` 檔或 ZIP 內容）。**Collection mode profile**（Standard / TriageFast / ForensicStrict）亦在 Settings 設定：`ForensicStrict` 會在收集前加強 live-response／輸出路徑警示，並**阻擋**上述 ZIP 上傳與 **AI Analyze**（非 zero-footprint／完整鑑識套件承諾）；`collection_coverage.json` 與 analyst 匯出會記錄本次 profile。
...
**記憶體採集（選用）**：於 **Advanced → Settings** 可開啟 `Memory acquisition` 區塊，指定外部工具路徑、**引數 preset**（`custom` = 下方欄位為完整模板；**非** `custom` = preset 先展開核心引數，再把下方欄位內容**接在後面**作額外參數，仍支援 `{OutputPath}` / `{OutputDir}`）。收集結果寫入 `memory_acquisition.json`，且出現在 `collection_coverage.json` 的 **Memory acquisition** 步驟；若 sidecar 宣稱 `complete` 但預期 dump 不存在，coverage 與寫入前的 sidecar reconcile 會標示失敗並以 `[Coverage]` / `[Reconciled]` 註記。Summary / HTML / `summary.json` / `full_log_v3` 會分開說明 `Coverage` 與 `Sidecar` 的語意。案卷內 `Memory\` 下為映像檔（可能很大）。本工具僅編排執行與稽核 metadata，不解析 dump。

**記憶體分析 handoff（選用）**：同一個 Settings 對話框內可再啟用 `Memory analysis handoff`，指定外部分析器、**dual-quoted / Custom preset** 與參考模板（`{InputPath}`、`{InputDir}`、`{OutputDir}`、`{CaseDir}`）。若案卷中已有 dump，平台會在收集階段把 dump 交給外部工具執行，並寫出 `memory_analysis.json` 與 `MemoryAnalysis\` 目錄下的工具輸出；輸出目錄必須是案卷下專用子目錄，平台會拒絕 case root 或既有保留證物目錄。寫入 sidecar 前會核對列舉的輸出檔是否仍存在於磁碟。Summary / HTML / `summary.json` / `full_log_v3` 只記錄 handoff 執行情況與輸出檔 accounting，並以統一說明區分 `Coverage` 與 `Sidecar`，不會把外部工具結果包裝成內建記憶體 verdict。
...
- [x] **Fact Store**: 載入案卷時預設自動建置（可於 Settings 關閉）；含 Process/**LogonSession / NetworkResource / ServerConnection / StoredCredential / KerberosTicketCache**/Autorun/**Service**/ActivityTimeline/MFT/ScheduledTask/EventLog/USN/BAM/DAM/BITS/**Jump List**/WMI Persistence/**Amcache/ShimCache Entries/SRUM Network/SRUM App/ShellBags/Memory acquisition / Memory analysis handoff** Normalizer、實體索引與查詢，並保留 `TimeKind` / `TimeConfidence`；每筆 fact 另帶 `CollectionStep / CollectionStatus / CollectionPrivilege / ParseLevel / FallbackUsed / SourceFile / RawRef / ParserNote` provenance，不在工具內直接下判斷。UI 另提供 **Rebuild Selected Host Fact Store**，供分析師在單機層級手動刷新。
...
- [x] **Guided Hunt Pack (v1)**: 在 facts-only 模型之上新增可關閉的 explainable overlay；目前提供 ATT&CK-mapped 規則、evidence lines 與 hypothesis templates，涵蓋 RDP、SMB admin share、BITS、WMI、Autorun、Scheduled Task、Service path 與 credential-backed remote access 等調查 lead；`GH-CRED-001` 會同時參考 Event Log、`stored_credentials.txt` (`cmdkey /list`) 與 `kerberos_tickets.txt` (`klist`) 等 facts，但不回寫 Fact Store、也不輸出 verdict。
- [x] **Timeline Analysis (全方位時間軸)**: Process + MFT + EventLog + USN + BAM/DAM + BITS + **Amcache + ShimCache Entries + SRUM Network/App + Activity**（activity_timeline.csv；其中 **Jump List** 跡證已由 builder 併入，`Source=JumpList`，**不**再對 `jump_lists.csv` 重複匯入 Timeline）合併為單一時間軸；支援 **Time Type / Confidence**、**Source / 時間區間篩選**、**Export CSV/JSON**。時間篩選會 honor 到分鐘，不再把 graph handoff 視窗擴大成整天。（原 Activity Timeline 分頁已移除，活動資料於此檢視。）
